...
describe("ServicePairPanel", () => {
  ...
  it("renders one row per channel, kind/label/verification badge and producer/consumer counts", async () => {
    (globalThis as any).fetch = fakeFetch({
      "/api/services/channels?from=rails-svc&to=vega-svc": {
        from: "rails-svc",
        to: "vega-svc",
        channels: [
          { kind: "publishes", channel: "vega_requests", edge_id: "e1", from: "rails-svc", to: "vega-svc", verification_state: "verified", producer_count: 1, consumer_count: 2 },
          { kind: "http_call", channel: "GET /status", edge_id: "e2", from: "vega-svc", to: "rails-svc", verification_state: "candidate", producer_count: 3, consumer_count: 1 },
        ],
      },
    });
    render(() => <ServicePairPanel from="rails-svc" to="vega-svc" />, container);
    ...
    expect(rows[0].textContent).toContain("vega_requests");
    ...
    expect(rows[0].textContent).toContain("rails-svc → vega-svc");
    expect(rows[1].textContent).toContain("GET /status");
    expect(rows[1].textContent).toContain("vega-svc → rails-svc");
  });
