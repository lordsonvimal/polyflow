...
// TestChains_RabbitMQCrossRepo traces the confirmed real chain: MainSvc (Rails)
// publishes through bunny with the exchange held in a variable, svc-c-agent (Go)
// consumes via amqp091; a workspace broker hint connects them.
func TestChains_RabbitMQCrossRepo(t *testing.T) {
	pubNodes, pubEdges := matchFixtureFiles(t, "main-svc",
		filepath.Join(patternsRoot, "ruby/bunny_test/input.rb"),
		filepath.Join(patternsRoot, "ruby/bunny.yaml"))
	subNodes, subEdges := matchFixtureFiles(t, "svc-c-agent",
	...
	r := Run(idx, root.ID, "forward", 0, false, 0, nil, 0)
	require.NotNil(t, r)
	require.NotEmpty(t, r.Chains)

	// Since J.3 the hint attaches to the consumer's own `dsw.builds` channel
	// instead of minting a parallel `broker:channel:dsw.builds`, so the service
	...
	require.NotEmpty(t, hit,
		"expected a chain main-svc publisher → dsw.builds channel → svc-c-agent consumer, got:\n%s",
		chainTexts(r))
