...
//
//	# nextGen — the server DECLARES the queue name in a REST response
//	amqp_progress_events_queue_name: cdr_progress_events_queue(organization)
//
//	# nextGen-CDR-Agent — the agent READS it back by field name at runtime
...
//     resolve a cross-file `cdr_progress_events_queue(org)` reference that Tier
...
func stampRubyHandshakeFacts(file string, src []byte, nodes []graph.Node) {
	...
	// nextGen's lib/queue_names.rb declares no channel and publishes nothing —
...
// it, so a queue-name module (nextGen's lib/queue_names.rb, whose methods are
...
// a handshake field — the `cdr_progress_events_queue` in
// `amqp_progress_events_queue_name: cdr_progress_events_queue(org)`. The
