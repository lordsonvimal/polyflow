...
func TestParameterizeQueueName(t *testing.T) {
	...
	cases := map[string]string{
		...
		"lyra-lro-events":       "lyra_lro_events",
...
`)
	...
	resolveRubyQueueKeys("vega_progress_event_worker.rb", src, nodes)

	assert.Equal(t, "vega_progress_events", nodes[0].Meta["queue_name"],
	...
	nodes := []graph.Node{
		{
			...
			Meta: map[string]string{
				"key_dynamic":     "true",
				"key_dynamic_raw": "vega_progress_events_queue(org)",
			},
		},
	}
