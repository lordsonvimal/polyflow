...
// fmt.Sprintf in the same function — the exact cdr_api_client.go /
// mysycamore_client.go shape Tier X.11 targets (docs/sprintf-url-resolution-plan.md).
...
// for the other half of the measured DSW→nextGen client surface: the URL is not a
...
//
// Modelled on dsw-manager/internal/clients/cdr_user_sync.go, which produced zero
...
func TestPathEvidence(t *testing.T) {
	cases := []struct {
		...
		// resolves in more than one service. On the datascience fleet that
