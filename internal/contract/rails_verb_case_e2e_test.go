...
// Rails route. Before HH.2 the Atlas route was keyed "post /login", so the *only*
...
func TestExactTierNotShadowedByVerbCase(t *testing.T) {
	...
	var atlasLogin string
	for _, n := range nodes {
		if n.Type == graph.NodeTypeHTTPHandler && n.Meta["path"] == "/login" {
			atlasLogin = n.ID
		}
	}
	require.NotEmpty(t, atlasLogin, "fixture must yield a Atlas /login handler")
	...
	nodes = append(nodes,
		...
		graph.Node{ID: "gin-login", Type: graph.NodeTypeHTTPHandler, Service: "willow",
			Label: "POST /login", File: "main.go",
			Meta: map[string]string{"method": "POST", "path": "/login"}},
		graph.Node{ID: "atlas-client", Type: graph.NodeTypeHTTPClient, Service: "atlas",
	...
	for _, e := range res.Edges {
		if e.From == "atlas-client" {
			targets[e.To] = e.Confidence
		}
	}
	require.Contains(t, targets, atlasLogin,
		"client must reach its own service's route; got %v", targets)
	assert.Equal(t, graph.ConfidencePartial, targets[atlasLogin],
