...
// nextGen-CAM's own React SPA posts to "/login". A relative URL in a browser
// bundle resolves against the origin that served it, so it reaches CAM's own
// route and cannot reach mysycamore's identically-named one — which is what it
// linked to at HEAD, at `static` confidence, because the exact tier indexes the
// raw key and CAM's Rails route carries a lowercase verb (see Tier HH.2).
func TestHTTPRule_BrowserRelativeURLStaysSameOrigin(t *testing.T) {
	nodes := []graph.Node{
		{ID: "c1", Type: graph.NodeTypeHTTPClient, Service: "nextgen-cam",
			Language: "typescript",
			Meta:     map[string]string{"method": "POST", "url": "/api/v1/users"}},
		{ID: "h1", Type: graph.NodeTypeHTTPHandler, Service: "nextgen-cam",
			Meta: map[string]string{"method": "POST", "path": "/api/v1/users"}},
		{ID: "h2", Type: graph.NodeTypeHTTPHandler, Service: "mysycamore",
			Meta: map[string]string{"method": "POST", "path": "/api/v1/users"}},
	}
	res := runHTTP(t, nodes, nil)
...
// however server-side the .templ file around it looks. mysycamore's launcher
// `@get('/apps/search')` names a route mysycamore itself serves; at HEAD it
// fanned out to dsw-manager's same-named route as well.
func TestHTTPRule_DatastarActionStaysSameOrigin(t *testing.T) {
	nodes := []graph.Node{
		{ID: "c1", Type: graph.NodeTypeHTTPClient, Service: "mysycamore",
			Language: "templ",
			Meta:     map[string]string{"method": "GET", "path": "/apps/search", "datastar": "true"}},
		{ID: "h1", Type: graph.NodeTypeHTTPHandler, Service: "mysycamore",
			Meta: map[string]string{"method": "GET", "path": "/apps/search"}},
		{ID: "h2", Type: graph.NodeTypeHTTPHandler, Service: "dsw-manager",
			Meta: map[string]string{"method": "GET", "path": "/apps/search"}},
	}
	res := runHTTP(t, nodes, nil)
	require.Len(t, res.Edges, 1, "mysycamore serves /apps/search itself")
	assert.Equal(t, "link:c1->h1", res.Edges[0].ID)
}

// The rule must not leak to server-side clients: dsw-manager's resty client
// posts the relative fragment "/api/v1/configs/remove" to dsw-agent, joined to
...
func TestHTTPRule_ServerSideRelativeURLStillCrossesServices(t *testing.T) {
	nodes := []graph.Node{
		{ID: "c1", Type: graph.NodeTypeHTTPClient, Service: "dsw-manager",
			Language: "go",
			Meta:     map[string]string{"method": "POST", "url": "/api/v1/configs/remove"}},
		{ID: "h1", Type: graph.NodeTypeHTTPHandler, Service: "dsw-agent",
...
func TestHTTPRule_BrowserAbsoluteURLStillCrossesServices(t *testing.T) {
	nodes := []graph.Node{
		{ID: "c1", Type: graph.NodeTypeHTTPClient, Service: "nextgen-cam",
			Language: "typescript",
			Meta:     map[string]string{"method": "GET", "url": "https://mysycamore.internal/api/v1/users"}},
		{ID: "h1", Type: graph.NodeTypeHTTPHandler, Service: "mysycamore",
			Meta: map[string]string{"method": "GET", "path": "/api/v1/users"}},
	}
	res := runHTTP(t, nodes, nil)
...
// Dedupe keys on the resolved key, not the line: mysycamore's launcher.templ
