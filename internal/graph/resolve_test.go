...
// TestResolveTarget_ExactIDShortCircuit reproduces the datascience AMQP bench
...
// dsw-agent producer channel instead, because ResolveTarget only ever matched
...
func TestResolveTarget_ExactIDShortCircuit(t *testing.T) {
	wanted := node("dsw-manager:channel:runner_heartbeat/heartbeat", "runner_heartbeat/heartbeat", "dsw-manager", "exchanges.go", "channel")
	wrong := node("dsw-agent:channel:runner_heartbeat/", "runner_heartbeat/", "dsw-agent", "", "channel")
	s := &stubSearcherWithGetNode{
