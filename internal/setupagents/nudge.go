...
- Once search/context/resolve/hierarchy gives you a node id for the symbol you actually need, call read with that id instead of opening the whole file — it returns just that function/method/class's exact source span, not the surrounding file, which is cheaper and keeps unrelated code out of context. If you need several symbols — even across different files — pass them all as read's targets list in one call instead of one read per symbol; a bad target in the list only fails that entry, not the whole call.
- After each polyflow tool result, once you have a clear judgment, print one visible line so the user can see whether the call paid off: ` + "`polyflow[<tool>]: <decisive|useful|thin|noisy|wrong> — <≤10 words>`" + ` (e.g. ` + "`polyflow[search]: noisy — 12 duplicate ERB nodes buried the hit`" + `). Skip the line if you're unsure rather than guessing, and never let rating a call change whether you make it.
- Once a search/investigate/read result gives you the specific file:line and its inline source snippet, treat that as ground truth — it's pulled directly from the file, not a paraphrase. Don't re-grep or re-Read the same location to double-check unless you need surrounding code the snippet didn't include, or the result was itself flagged low-confidence/weak-match. Re-verifying a polyflow hit by hand defeats the tool's purpose.`
