...
// internal/indexer/link_passes.go's apply_hints_and_enrich pass does:
// convert workspace.Link -> graph.LinkHint, run the framework globally over
// every node, then merge its patches onto a working copy. These full-pipeline
// e2e tests build the link-pass sequence by hand rather than going through
// indexer.runIndex, so each one needs its own call site instead of the one
