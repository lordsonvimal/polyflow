...
//   main.go      → parseable
//   deploy.sh    → unparsed blind spot (.sh)
...
func buildFixtureTree(t *testing.T) string {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "deploy.sh"), []byte("#!/bin/sh"), 0o644))
...
func TestWalkService_UnparsedCounts(t *testing.T) {
	...
	assert.Equal(t, 1, unparsed[".sh"], ".sh count")
...
func TestSerializeUnparsed_Determinism(t *testing.T) {
	...
	assert.Equal(t, 3, parsed["api"][".sh"])
...
func TestUnparsedSummary_TopThree(t *testing.T) {
	...
	// Sorted: .sh, .svelte, .vue, Dockerfile → top-3: .sh, .svelte, .vue
	assert.Contains(t, parts, ".sh", "top-3 should include .sh")
...
func TestWalkService_ParseableFilesNotCounted(t *testing.T) {
	...
	assert.Equal(t, map[string]int{".sh": 1}, unparsed, "only .sh is a blind spot")
}
