...
func TestRun_UnparsedFileLedger(t *testing.T) {
	...
	writeFile(t, svcDir, "deploy.proto", "// not parsed, just an unparsed blind spot\n")
	...
	// Verify content: .proto and Dockerfile present; .png absent.
	var parsed map[string]map[string]int
	require.NoError(t, json.Unmarshal([]byte(meta1), &parsed))
	apiCounts := parsed["api"]
	assert.Equal(t, 1, apiCounts[".proto"], ".proto count in api service")
