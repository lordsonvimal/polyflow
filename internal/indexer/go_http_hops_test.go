...
func buildThreeHopWorkspace(t *testing.T) *workspace.WorkspaceConfig {
	...
	writeFile(t, svcA, ".env", "DSW_MANAGER_API_URL=http://dsw-manager.internal:3100/api/v1\n")
...
`)
	writeFile(t, svcA, "dsw_manager_user_app_client.go", `package main
...
func (c *DSWManagerUserAppClient) CreateUserApp() error {
...
`)
	...
	return &workspace.WorkspaceConfig{
		...
		Services: []workspace.Service{
			...
			{Name: "dsw-manager", Path: svcB, Language: "go"},
		},
	}
}
...
func TestGoHTTPHostHops_ForwardedParamLinksAcrossServices(t *testing.T) {
	...
	assert.Equal(t, "DSW_MANAGER_API_URL", client.Meta["env_var"],
	...
	require.NotEmpty(t, handlerID, "dsw-manager must expose POST /api/v1/user-apps")
	...
	assert.True(t, linked,
		"the three-hop env trace plus the config-supplied prefix must link migrator → dsw-manager")
}
