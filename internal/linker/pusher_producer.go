...
// EnrichPusherHelperCalls is PU.2e — the `pusher(msg, status)` helper
// indirection. Dozens of controllers / concerns / importers / jobs call a local
// `def pusher` (or `BaseImporter.non_instance_pusher`) that forwards to
// `PusherClient.new(...).push` → `.trigger`. The def and the call routinely live
// in different files (a concern `include`d into a controller), so no same-file
// walk connects them, and every call site otherwise dead-ends at an unresolved
// `pusher` call_ref. This wires each such call's enclosing method straight to
// the canonical `PusherClient` trigger publisher.
//
// It is separate from EnrichPusherProducers because that pass bails when the
// wrapper has no `notify_* -> event` map (cedar's PusherClient forwards a runtime
// event), whereas this indirection only needs the trigger node to exist.
func EnrichPusherHelperCalls(nodes []graph.Node, serviceFiles map[string][]string) []graph.Edge {
	canon := canonicalPusherTriggerPublishers(nodes)
	if len(canon) == 0 {
		return nil
	}
	...
	for _, fs := range serviceFiles {
		for _, f := range fs {
			if strings.HasSuffix(f, ".rb") && !graph.IsTestFilePath(f) {
	...
	helperDefNames := map[string]bool{}
	type pendingCall struct {
		method, relFile string
		line            int
	}
	...
	for _, f := range files {
		...
		collectPusherHelperDefs(fa, helperDefNames)
		rel := patterns.RelativizeToCwd(f)
		for _, hc := range collectPusherHelperCalls(fa) {
			pending = append(pending, pendingCall{hc.method, rel, hc.line})
		}
		fa.release()
	}
	if len(helperDefNames) == 0 {
		return nil
	}
	...
	for _, pc := range pending {
		if !helperDefNames[pc.method] {
			continue
		}
		fn := enclosingFn(pc.relFile, pc.line)
		if fn == "" {
			continue
		}
		for _, pub := range canon {
			key := fn + "\x00" + pub
			if emitted[key] {
				continue
			}
			...
			edges = append(edges, graph.Edge{
				ID:         fmt.Sprintf("%s->%s:pusher_helper_call", fn, pub),
				From:       fn,
				To:         pub,
				Type:       graph.EdgeTypeCalls,
				Confidence: graph.ConfidenceInferred,
				Meta:       map[string]string{"via": "pusher_helper_call"},
			})
		}
	}
...
// collectPusherHelperDefs adds every `def pusher` / `def non_instance_pusher`
// whose body constructs a `PusherClient` to names.
func collectPusherHelperDefs(fa *rubyFileAST, names map[string]bool) {
	var walk func(n *sitter.Node)
	walk = func(n *sitter.Node) {
		if n.Type() == "method" || n.Type() == "singleton_method" {
			if nm := n.ChildByFieldName("name"); nm != nil {
				mName := nm.Content(fa.src)
				if (mName == "pusher" || mName == "non_instance_pusher") &&
					strings.Contains(n.Content(fa.src), "PusherClient") {
					names[mName] = true
				}
			}
		}
...
type pusherHelperCall struct {
	method string
	line   int
}

// collectPusherHelperCalls returns every bare `pusher(msg, status, …)` /
// `<Const>.non_instance_pusher(…)` call — at least two positional args, so a
// zero-arg `pusher` reader / a 1-arg `pusher(notification)` sink don't match.
func collectPusherHelperCalls(fa *rubyFileAST) []pusherHelperCall {
	var out []pusherHelperCall
	var walk func(n *sitter.Node)
	walk = func(n *sitter.Node) {
		if n.Type() == "call" {
			if mn := n.ChildByFieldName("method"); mn != nil {
				m := mn.Content(fa.src)
				if m == "pusher" || m == "non_instance_pusher" {
					recv := n.ChildByFieldName("receiver")
					okRecv := recv == nil || recv.Type() == "self" || recv.Type() == "constant" ||
						recv.Type() == "scope_resolution"
					if okRecv && pusherArgAt(n, 1) != nil {
						out = append(out, pusherHelperCall{m, int(n.StartPoint().Row) + 1})
					}
				}
			}
		}
...
// canonicalPusherTriggerPublishers returns the `pusher_trigger*` publisher
// node(s) inside the `PusherClient` wrapper file — the single sink every
// `pusher(...)` helper ultimately reaches. Falls back to every `pusher_trigger*`
// publisher when no file is recognisably the wrapper.
func canonicalPusherTriggerPublishers(nodes []graph.Node) []string {
	var canon, all []string
	for i := range nodes {
		n := &nodes[i]
		if n.Type != graph.NodeTypePublisher {
			continue
		}
		if p := n.Meta["pattern"]; p != "pusher_trigger" && p != "pusher_trigger_async" {
			continue
		}
		all = append(all, n.ID)
		if base := n.File[strings.LastIndexByte(n.File, '/')+1:]; strings.Contains(base, "pusher_client") {
			canon = append(canon, n.ID)
		}
	}
	if len(canon) > 0 {
		sort.Strings(canon)
