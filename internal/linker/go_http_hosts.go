...
//
//	config.go:234        WillowAPIURL: os.Getenv("MYSYCAMORE_API_URL")
//	main.go:1242         c := NewWillowClient(config.WillowAPIURL, key)
//	willow_client.go:24   return &WillowClient{baseURL: strings.TrimSuffix(baseURL, "/")}
//	willow_client.go:51   reqURL := fmt.Sprintf("%s/api/v1/users?email=%s", c.baseURL, …)
//
	...
	for _, f := range files {
		ast.Inspect(f, func(n ast.Node) bool {
			...
			for i, arg := range call.Args {
				env := directEnvOf(arg)
				if env == "" {
					// `config.WillowAPIURL` → the field's env var.
...
// (`config.WillowAPIURL` → WillowAPIURL, `c.baseURL` → baseURL), or "".
