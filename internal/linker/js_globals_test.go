...
func TestExtractHandlerCandidates(t *testing.T) {
	...
	cases := []struct {
		...
		{"window.dsw.closeVulnerabilityModal()", []string{"window.dsw.closeVulnerabilityModal", "closeVulnerabilityModal", "window"}},
		...
		{"window.dsw[name]()", nil},  // computed member → dynamic dispatch
	}
...
// TestLinkJSGlobals_NamespacedHandler: dom_target handler="window.dsw.save()"
...
func TestLinkJSGlobals_NamespacedHandler(t *testing.T) {
	...
	saveFn := graph.Node{
		...
		Meta: map[string]string{"global_symbol": "save", "global_path": "window.dsw.save"},
	}
	listener := graph.Node{
		...
		Meta: map[string]string{"handler": "window.dsw.save()", "prop": "onclick", "pattern": "dom_event_attr"},
	}
