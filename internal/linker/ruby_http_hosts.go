...
// @sce_host → ENV). The env var remains the gate: a method with a path but no
...
func (fa *rubyFileAST) fileNamePath() map[string]string {
	...
	// (@service_base_url = "#{sce_host}/service_api/v1" needs @sce_host first).
...
//
// An env read returns ("", true) rather than false: `ENV.fetch("SCE_HOST")` is a
// host, and a host contributes a real and *empty* path — that is what lets
// "#{sce_host}/client_api/v1/agents/register" reduce to the bare route.
func (fa *rubyFileAST) pathTemplate(n *sitter.Node, namePath map[string]string, params map[string]bool, depth int) (string, bool) {
	...
	case "call", "method_call":
		...
		case "URI", "Pathname":
			// `URI("#{sce_app}/#{endpoint}")` — a wrapper whose sole argument is
