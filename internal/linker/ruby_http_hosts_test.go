...
module Server
  module ConnectionInfo
    def server_api_url(endpoint)
      sce_app = if Rails.env.test?
                  "http://localhost:8080"
                else
                  ENV.fetch("SCE_APP", nil)
                end
      URI("#{sce_app}/#{endpoint}")
    end
...
`)
	...
	if got := nodes[0].Meta["key_dynamic_raw"]; got != `ENV.fetch("SCE_APP")` {
		t.Errorf("key_dynamic_raw = %q, want ENV.fetch(\"SCE_APP\")", got)
	}
...
module Server
  module ConnectionInfo
    def server_api_uri
      URI.join(ENV.fetch("SCE_HOST", nil), "client_api/v1/agents/register")
    end
  end
end
`)
...
`)
	...
	if got := nodes[0].Meta["host_env_var"]; got != "SCE_HOST" {
		t.Errorf("host_env_var = %q, want SCE_HOST (param→caller trace)", got)
	}
}
...
// Connection singleton pattern from the real SCE agent.
...
class Connection
  def initialize
    @sce_host = ENV.fetch("SCE_HOST")
    @service_base_url = "#{sce_host}/service_api/v1"
  end
  attr_accessor :service_base_url, :sce_host
...
`)
	...
	alreadyDone := httpClientNode(base, 4, `ENV.fetch("SCE_APP")`)
