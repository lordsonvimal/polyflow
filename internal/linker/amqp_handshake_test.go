...
func TestAMQPHandshake_ResolvesQueueAcrossRepos(t *testing.T) {
	...
	assert.Equal(t, "vega_progress_events", pub.Meta["queue_name"],
...
// regression: orion-vega-agent/lib/client/agent_node.rb reads
...
func TestAMQPHandshake_ContractClosesPublisherToConsumer(t *testing.T) {
	...
	for _, e := range found {
		...
		if to := byID[e.To]; to != nil && to.Meta["queue_name"] == "vega_progress_events" {
			reachedWorker = true
