...
// nextGen-CDR-Agent PUTs /client_api/v1/lros/:id, which `resources :lros`
...
func TestLinkRailsRouteActions_WorkedExample(t *testing.T) {
	...
	nodes := []graph.Node{
		h,
		railsAction("nextGen", ctrl, "update", 58),
		railsAction("nextGen", ctrl, "create", 14),
	}
...
// namespace derivation load-bearing rather than cosmetic: nextGen really does
...
func TestLinkRailsRouteActions_NamespaceDisambiguates(t *testing.T) {
	...
	h := railsHandler("nextGen", "PATCH /client_api/v1/folders/:folder_id/files/:id", 610, map[string]string{
		"action":   "update",
		"resource": "files",
	...
	nodes := []graph.Node{
		h,
		railsAction("nextGen", nsCtrl, "update", 40),
		railsAction("nextGen", rootCtrl, "update", 12),
	}
...
func TestLinkRailsRouteActions_AmbiguousRefuses(t *testing.T) {
	...
	nodes := []graph.Node{
		h,
		railsAction("nextGen", "/repo/app/controllers/admin/reports_controller.rb", "index", 5),
		railsAction("nextGen", "/repo/app/controllers/client_api/v1/reports_controller.rb", "index", 9),
	}
...
func TestLinkRailsRouteActions_APIControllerWithoutNewStaysUnresolved(t *testing.T) {
	...
	h := railsHandler("nextGen", "GET /client_api/v1/users/new", 44, map[string]string{
		"action":   "new",
		"resource": "users",
	...
	nodes := []graph.Node{
		h,
		railsAction("nextGen", apiCtrl, "index", 10), // API controller exists…
		railsAction("nextGen", apiCtrl, "show", 20),  // …but declares no `new`
		railsAction("nextGen", rootCtrl, "new", 33),  // the HTML controller does
	}
...
func TestLinkRailsRouteActions_NoNameSimilarityFallback(t *testing.T) {
	...
	nodes := []graph.Node{h, railsAction("nextGen", elsewhere, "index", 12)}
...
func TestLinkRailsRouteActions_VerbRoutes(t *testing.T) {
	...
	member := railsHandler("nextGen", "POST /studies/:study_id/workspaces/:id/subscribe", 300, map[string]string{
		"action":    ":subscribe",
		"full_path": "/studies/:study_id/workspaces/:id/subscribe",
		"path":      "/studies/:study_id/workspaces/:id/subscribe",
		"pattern":   "member_verb_route",
	})
	collection := railsHandler("nextGen", "POST /client_api/v1/files/copy", 410, map[string]string{
		"action":    ":copy",
	...
	nodes := []graph.Node{
		member, collection,
		railsAction("nextGen", workspaces, "subscribe", 77),
		railsAction("nextGen", files, "copy", 120),
	}
...
func TestLinkRailsRouteActions_BeforeActionIsNotAnAction(t *testing.T) {
	...
	h := railsHandler("nextGen", "DELETE /sessions/:id", 15, map[string]string{
		"action":   "destroy",
		"resource": "sessions",
		"path":     "/sessions/:id",
		"pattern":  "rest_resource_route",
...
func TestLinkRailsRouteActions_OneEdgePerRoute(t *testing.T) {
	...
	dup := railsAction("nextGen", ctrl, "index", 10)
	dup.Type = graph.NodeTypeMethod
	dup.ID += ":m"

	edges, _ := LinkRailsRouteActions([]graph.Node{h, railsAction("nextGen", ctrl, "index", 10), dup})
