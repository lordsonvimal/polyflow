...
func TestMixinMethods_ResolvesThroughInclude(t *testing.T) {
	...
	assert.Equal(t, []string{
		"nextGen:/repo/app/controllers/files_controller.rb:function:show:10 -> " +
			"nextGen:/repo/lib/dx.rb:function:logger_context:31",
	}, edgeTargets(got))
...
func TestMixinMethods_NeverCrossesAServiceBoundary(t *testing.T) {
	...
	for i := range bNodes {
		...
		bNodes[i].ID = "nextgen-cdr:" + bNodes[i].File + ":" +
...
func TestMixinMethods_UnrelatedClassStaysUnresolved(t *testing.T) {
	...
	refs := []graph.UnresolvedRef{callRef("nextGen", "/repo/app/models/thing.rb", 6, "logger_context")}
...
func TestMixinMethods_ClassBodyCallIsAttributedToTheClass(t *testing.T) {
	...
	refs := []graph.UnresolvedRef{
		callRef("nextGen", "/repo/app/controllers/files_controller.rb", 5, "logger_context"),
	}
	got, _, _ := LinkRubyMixinMethods(nodes, edges, refs)
	require.Len(t, got, 1)
	assert.Equal(t, "nextGen:/repo/app/controllers/files_controller.rb:class:FilesController", got[0].From)
}
