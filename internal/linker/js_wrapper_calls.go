...
func scanJSWrapperCallSites(service, file string, wrappers map[string]int, bodyMethods map[string]string) ([]graph.Node, []graph.UnresolvedRef) {
	...
	for {
		...
		// function (`function load(url) { return apiGet(url); }`), filled by that
		// function's own callers further down the file. It can equally be a local
		// the KeyWalker declined to follow. Both are one question — what string
		// can this identifier be — and VG.6 asks the value engine rather than a
		// second backtracker. Anything ambiguous or dynamic stays dynamic: these
		// are frequently runtime values (`entry.url` from a fetched payload).
		if meta["key_dynamic"] == "true" && urlArg.Type() == "identifier" {
			if lit, ok := resolveWrapperArgURL(relFile, src, root, urlArg); ok {
				delete(meta, "key_dynamic")
				delete(meta, "key_dynamic_raw")
				meta["url"] = lit
				meta["url_via"] = "enclosing_param_forward"
				meta["vg_layer"] = vgLayer
				meta["vg_rule"] = vgCallSiteRule
				label = lit
			}
		}
