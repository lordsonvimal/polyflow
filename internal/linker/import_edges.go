...
// resolveJSImportPath is internal/jsast.ResolveImportPath — extracted there
// (FX.8.33) so internal/factpipe's rails_views hub can share it without
// duplicating it (factpipe cannot import internal/linker: the transitive
// cycle factpipe -> linker -> patterns -> factpipe).
func resolveJSImportPath(importingFile, specifier string, indexedFiles map[string]bool) string {
	return jsast.ResolveImportPath(importingFile, specifier, indexedFiles)
}
