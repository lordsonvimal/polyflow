...
// nextGen's application.scss: an explicit `.scss`, a `.css`, a partial
...
// cost 33 of nextGen's 40 import edges: LinkContainment mints a file node only
...
func TestLinkContainment_ContainsStylesheetSelectors(t *testing.T) {
	const svc, sheet = "nextGen", "/app/assets/stylesheets/issues.scss"
	nodes := []graph.Node{
		{
