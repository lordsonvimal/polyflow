...
// nextGen's data_server_communicator.rb: a shared `rest_request` wrapper whose
...
func TestResolveRubyWrapperURLCallSites_ResolvesDirectMergeShape(t *testing.T) {
	...
	nodes := parseRubyWrapperFixture(t, "nextGen", paths)
	serviceFiles := map[string][]string{"nextGen": paths}
