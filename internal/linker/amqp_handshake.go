...
// is shared by the two repos. orion hands the agent a per-organization queue
...
//
// and orion-vega-agent reads it back out of the config that response filled:
//
...
// resolves to a literal (`vega_progress_events`), which is the same literal the
...
//
//	orion-vega-agent            orion
//	channel.queue(dynamic)  ~~~  amqp_progress_events_queue_name:  →  "vega_progress_events"
//	  key_dynamic                  vega_progress_events_queue(org)        ↑
