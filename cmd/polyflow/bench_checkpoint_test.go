...
func measuredResult(taskID, arm string) agentbench.TaskResult {
	return agentbench.TaskResult{
		TaskID: taskID, Repo: "orion", CaseID: "c", Arm: arm, Trial: 1,
...
func TestCheckpoint_ResumeSkipsBoughtTrials(t *testing.T) {
	dir := t.TempDir()
	c, err := loadCheckpoint(dir, "orion", testHeader(), false)
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if c.count() != 0 {
	...
	c.record(measuredResult("orion/a", agentbench.ArmWithSemantics))
	c.record(measuredResult("orion/a", agentbench.ArmNoPolyflow))

	c2, err := loadCheckpoint(dir, "orion", testHeader(), false)
	if err != nil {
	...
	got, ok := c2.completed("orion/a", agentbench.ArmWithSemantics, 1)
	if !ok {
	...
	if _, ok := c2.completed("orion/b", agentbench.ArmWithSemantics, 1); ok {
...
func TestCheckpoint_FailedTrialsAreReplayed(t *testing.T) {
	...
	c.record(agentbench.TaskResult{
		TaskID: "orion/a", Arm: agentbench.ArmWithSemantics, Trial: 1,
	...
	if _, ok := c2.completed("orion/a", agentbench.ArmWithSemantics, 1); ok {
...
func TestCheckpoint_RefusesMismatchedRun(t *testing.T) {
	...
	if _, err := loadCheckpoint(dir, "orion", other, false); err == nil {
	...
	c3, err := loadCheckpoint(dir, "orion", other, true)
	if err != nil {
...
func TestCheckpoint_SurvivesATornLastLine(t *testing.T) {
	...
	if _, err := f.WriteString(`{"task_id":"orion/b","ar`); err != nil {
...
func TestCheckpoint_PerRepoIsolation(t *testing.T) {
	dir := t.TempDir()
	if a, b := checkpointPath(dir, "orion"), checkpointPath(dir, "chessleap"); a == b {
