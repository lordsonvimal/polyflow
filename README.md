...
| `polyflow deadcode` | Function/method/variable nodes with no live inbound edge, plus dead Rails views, excluding recognized entry points. `--include-types` adds unreferenced `struct`/`interface`/`type_alias` declarations; `--transitive` also flags code reachable only from other dead code (sound on Go/TS, a lead only on Ruby). `--service`/`--file` scope a large fleet scan. |
...
| `deadcode` | Function/method/variable nodes with no live inbound edge, excluding recognized entry points — a candidate list for removal, not a certainty (dynamic dispatch, reflection, and exported public API all show up as false positives). `include_types` adds unreferenced type declarations; `transitive` flags code reachable only from other dead code (sound on Go/TS, a lead only on Ruby). |
