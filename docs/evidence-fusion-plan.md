...
### Phase F.3 — Config/IaC resolution source `done`

**Outcome.** Implemented exactly as specified. `internal/evidence/config_resolve/` package contains `env.go` (dotenv reader — `.env`, `.env.*`, `*.env` files), `k8s.go` (Kubernetes Deployment YAML container-env reader, walks k8s/kubernetes/deploy/deployment subdirs), `terraform.go` (tfvars reader, walks terraform/infra/infrastructure subdirs), and `provider.go` (ConfigProvider, Name()="config"). Three pinned resolution semantics enforced: (a) all raw config values pass through `stripConfigValue` to strip surrounding quotes and whitespace (bug-class rule 6); (b) same var in multiple env overlays fans out to one edge per distinct value (bug-class rule 1; `TestEnv_FanOutMultipleEnvFiles` asserts ≥2 edges for API_URL present in both `.env` and `.env.production`); (c) env vars not found in any scanned config stay in the ledger as `config_not_found` rather than being guessed. `Evidence.ClearsUnresolved` field added to clear original `dynamic_<kind>` ledger entries when handled (reconciler filters them from the static unresolved set). `computeState` in `reconcile.go` extended to treat `config` as a confirming source alongside `runtime` and `contract` — so a static edge confirmed by config resolution flips to `verified`. Env var name extraction supports: `os.Getenv("VAR")`, `process.env.VAR`, `ENV["VAR"]`, `ENV.fetch("VAR")`, `os.environ.get("VAR")`, `System.getenv("VAR")`, and bare SCREAMING_SNAKE_CASE identifiers; all other expressions fall through to `config_not_found`. The `ConfigProvider` is wired into the indexer's reconciler pass (alongside staticProv and contractProv). Tests cover: HTTP edge resolution from .env, fan-out across multiple env files, two-node fan-out, Kafka edge, k8s manifest, terraform tfvars, ClearsUnresolved for both resolved and unresolved cases, no-config degradation, nil workspace, unextractable variable, quote stripping, determinism (two-run byte-identical), and reconciler integration (config verifies a static edge + dynamic_url entry cleared). No SchemaVersion bump required (no stored node/edge shape change). BenchmarkIndexCold: 10.2s for 1200 files (~8.5 ms/file), identical to pre-F.3 baseline.
