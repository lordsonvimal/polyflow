...
### Phase L.P1 — Python dependency resolution `done`
...
**Outcome (2026-07-18).** Implemented exactly as specified. `EcosystemPyPI = "pypi"` added to `internal/deps/deps.go`; `internal/deps/python.go` adds `resolvePython(dir)` (wired into `Resolve()`) covering seven requirements file names (`requirements.txt` → prod, `requirements-dev.txt` / `requirements_dev.txt` / `dev-requirements.txt` / `requirements-test.txt` / `test-requirements.txt` → dev, `constraints.txt` → prod), `resolvePoetryLock` (TOML line scanner; `category = "main"` → prod, `category = "dev"` → dev; poetry v2 `groups = ["dev"]` handled; fallback to pyproject.toml dev-group lookup when category absent), and `resolveUVLock` (TOML line scanner; kind from pyproject.toml `[dependency-groups.dev]` / `[tool.uv.dev-dependencies]` sections). `scanTOMLPackages` is a minimal TOML scanner (no external dependency added — same no-dep policy as Gemfile.lock/yarn.lock parsers). Package names normalized per PEP 503 (`normalizePyPIName`: lowercase, `_`/`.` → `-`). `reqLineRe` matches lines with any specifier operator; exact `==` pins and best-effort base-version for ranges (same policy as npm's `stripRange`). 7 tests in `internal/deps/python_test.go` (requirements.txt, poetry.lock, poetry v2 groups, uv.lock + pyproject.toml dev groups, uv.lock + `[tool.uv.dev-dependencies]`, name normalization, empty-dir negative). `TestForService_PythonVersionGating` added to `internal/patterns/version_test.go` (flask 2.3.0 activates gated pattern, flask 1.x suppresses it, absent flask suppresses it). `SchemaVersion` NOT bumped — no stored node/edge shape change. `BenchmarkIndexCold` held at ~11.1s / 1200 files.

### Phase L.P2 — Python HTTP frameworks → contract engine `done`
...
**Outcome (2026-07-18).** Implemented exactly as specified. Five package-gated pattern YAML files added (zero code changes): `patterns/python/fastapi_routes.yaml` (`package: fastapi` — one pattern `fastapi_route` for `@*.get/post/put/patch/delete/head/options("path") def handler()`), `patterns/python/flask_routes.yaml` (`package: flask` — two patterns: `flask_route` for `@app.route("path")` and `flask_method_route` for `@app.get/post/...("path")`), `patterns/python/django_routes.yaml` (`package: django` — one pattern `django_url_route` for `path()/re_path()/url()` calls in urls.py), `patterns/python/requests_client.yaml` (`package: requests` — two patterns: `requests_call` for verb methods, `requests_request` for `.request("METHOD", url)`), `patterns/python/httpx_client.yaml` (`package: httpx` — two patterns: `httpx_request` for verb methods, `httpx_request_method` for `.request("METHOD", url)`). All pattern names classify correctly via existing `classifyPattern` heuristics (route/handler → HTTPHandler; request/client-containing → HTTPClient) with no changes to matcher.go. Each YAML file has a fixture dir with positive (`input.py` + `expected.json`) and negative (`negative.py`) files; all 5 `TestPatternFixtures` subtests pass. 2-service integration test at `internal/e2e/python_go_test.go` uses `NewTreeSitterMatcherForService` with per-service deps resolved from `requirements.txt`/`go.mod`, runs full indexer + ApplyHints + contract engine, and asserts cross-service `http_call` edges from svc-python (`requests.get("http://svc-go/users")`) to svc-go (`r.GET("/users", listUsers)`). `TestPythonGo_CrossServiceHTTPCall` passes and includes a determinism run (rule 2). Full test suite passes. `SchemaVersion` NOT bumped — no new node/edge types (http_handler/http_client/http_call already existed); only new pattern YAML files. `BenchmarkIndexCold` held (pattern files are loaded once at registry init, not on the hot indexing path). Deviations: (1) `flask_route` pattern does not extract `methods=[...]` from Flask `@app.route` keyword argument (list element extraction is outside tree-sitter query language capability); handlers registered with `@app.route` have empty `meta["method"]`, which the contract engine matches via the `""` method_fallback tier — correct for GET-only Flask default routes. (2) `httpx_client.yaml` pattern name prefix `httpx_request` satisfies the `strings.Contains(..., "request")` classifyPattern heuristic without any code change. (3) `python/fastapi_routes_test/negative.py` uses `@staticmethod` and `@property` decorators (non-call form) which don't match the `(decorator (call ...))` pattern, confirming no false positives on decorator-only functions.

### Phase L.P3 — Python messaging + eval repo `done`
...
### Phase L.W2 — jQuery/AJAX cross-service links + selector→DOM-node linking `done`
...
### Phase C.2 — Evidence freshness labeling `done`
...
**Outcome.** `AgeString` + `CompactSourcesAt` annotate runtime sources with
human-readable age. `BuildVerificationSummaryAt(edges, staleAfter, now)` counts
stale verified edges in `StaleEvidence` and adds "stale runtime evidence" to the
note without changing any `VerificationState`. Workspace `evidence.stale_after`
(default 30d) threads through all four Build functions via `loadStaleAfter`.
`status` lists sessions with ages via `ListSessionInfos`; `doctor` warns when all
verified edges are stale. 1136 tests pass.
