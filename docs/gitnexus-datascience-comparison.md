# Polyflow vs. GitNexus — 10 Use Cases on `juniper`

Live, executed comparison against [GitNexus](https://github.com/abhigyanpatwari/GitNexus) on the
`juniper` fleet repo (maple-agent + maple-manager, 1086 files). Both tools indexed fresh.
...
**2. Cross-service HTTP trace** (`RegisterAgent` in maple-agent → `Heartbeat` handler in
maple-manager)
...
- polyflow: deep multi-hop view — methods, callers (incl. tests and `main`), and downstream to
  ...
  "Cross-service" section prints `→ contains → maple-manager` 19 times (same-service containment
...
"maple-manager", "edge_type": "contains"}` — same-service `contains` edges with an unset source
